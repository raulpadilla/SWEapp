...
//top.SiebelApp = SiebelApp;

var SiebelAppFacade = SiebelAppFacade || {};
//top.SiebelAppFacade = SiebelAppFacade;
...
if( !(/(iPhone|iPod|iPad)/i.test(navigator.userAgent)) && (!IsOfflineModeEnabled() ||!(window.localStorage.getItem("WorkOffline") &&JSON.parse(window.localStorage.getItem("WorkOffline")))) ){
...
function ProcessRefresh (){
   ...
   if( browserUrl.indexOf("SWECmd=GetCachedFrame") == -1 ){
        if(typeof(top.strWindowId) != "undefined"){
            var expiry = new Date(); 
...
function SWESubmitForm (csFormObj, csObj, fieldName, rowId, csFrame)
{
   ...
   if( IsOpenUI() && csFormObj.name === "SIPopupIEForm"){ // File Import Export Form
       ...
       $( "body" ).find( "iframe#ouitemp_iframe" ).remove().end().append( "<iframe id='ouitemp_iframe'  name='ouitemp_iframe' />" );
       ...
       $('#ouitemp_iframe').load(function () {
           ...
           if (typeof updatePopup == 'function') {
               updatePopup($('body', this.contentWindow.document).html());
           }
           setTimeout(function () {$('#ouitemp_iframe').remove();}, 50);
       }); 
    }
...
function IsOpenUI(){
    return SiebelApp!=undefined && SiebelApp.S_App!=undefined;
}
...
function HandleiOSRefresh(){
    if( /(iPhone|iPod|iPad)/i.test(navigator.userAgent) ){
        ...
        if( isCookie ){
            ...
            expDate.setTime(expDate.getTime() + 1001);     //Reset cookie timeout to 1s from now (minimum increament).
