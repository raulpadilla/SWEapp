   ...
   { 
      case "click":
         if (typeof (psItem) != "undefined")
         {
            ...
            if (command != "" && enabled)
            {
               rVal = top._swe._sweapp.S_App.GetCommandMgr().InvokeUtilityLinkCommand (App(), command);
            }
         }
         ...
         {
            rVal = top._swe._sweapp.S_App.GetCommandMgr().InvokeCommand (item.cmdId);
         }
         break;
   }
   return rVal;
...
function JSSToolbar_DoUpdateItemState (toolbar, idx)
{
   ...
   var s_cmdmgr = top._swe._sweapp.S_App.GetCommandMgr();
   // canInvoke state:
