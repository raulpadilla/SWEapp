...
var style_btnbar = "background-color:#ccccff; height:31px;";
var style_btn = "border:1px solid #ccccff;";
...
var style_btnOut = "border:1px solid #ccccff;";
...
var style_timer = "border:0px; background:#ccccff;";
...
function Initiate_HTMLToolbar(toolbarID)
{
   ...
   {
      var sHTML = "";
      sHTML += "<table border=0 CELLPADDING=0 CELLSPACING=0 width=100% bgcolor=#ccccff height=33>";
...
function CreateHTMLToolbar()
{
	...
	if (tbar)
	{
		...
		{
			sHTML += "<td id=\""+controlList[i].id+" TD\" style=\"border:1px solid #ccccff;\">"+controlList[i].HTML+"</td>";
		}
...
function HTMLControl(type, name, values, enabled, image, imaged, width, sequence, index)
{
	...
	var name_state = this.name + ". " + this.state;
	var sHTML = "";
	if (type=="Button")
	{
		var cb = new Array(3);
	...
	{
	   sHTML += "<LABEL for=\"" + this.id + "\" style=\"display:none\">" + this.name + "</LABEL>";
		sHTML += "<SELECT id=\"" + this.id + "\" name=\"" + this.name + "\" ";
		...
		if (property=="state")
		{
			if (img!=null)
			{
			   changeBorder(this.id, -1);
				...
				else if (this.state == "Blinking") control_blink(this.id);
				if (this.state!="unToggled")
					img.alt = this.name + ". " + this.state;
			}
			if (this.state!="unToggled")
			{
...
function InvokeToolbarCommand(command, ID) {
	...
	for (i=0; i<controlListIndex; i++)
	{
		...
		{
			control_value += "&" + controlList[i].name + "=" + controlList[i].obj.value;
