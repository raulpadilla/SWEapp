...
{   
   ...
   init : function( editor )
   {
        ...
        var arrGroupName=[];
		var valueToInsert;
        arrGroupName = window.g_arrCategory;
	   ...
	   socialSharingImagePath[8] = this.path +'delc16.jpg';
	    ...
	    var buildList = function buildList( ) {
		    ...
		    for(j=0;j<arrGroupName.length;j++)
		    {
				    ...
				    for (i=0;i<window.g_arrItem[j].length;i++){
					    combo.add(window.g_arrItem[j][i]);
				    }
		    }	
        };
      ...
      editor.ui.addRichCombo( 'mergefields',
         {
            ...
            { 
			   ...
			   for (i=0; i<window.g_arrCategory.length;i++){
				  this.add(i+1,window.g_arrCategory[i],window.g_arrCategory[i]);
               }
            },
		 ...
		 var insertValue= {
		            exec : function() {
						...
						}catch(Exception){alert('Please select value');}
				        editor.fire( 'saveSnapshot' );
					}
